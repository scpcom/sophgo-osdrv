...
#include <linux/isp_reg.h>
#include <linux/vi_reg_blocks.h>
#include <linux/vi_reg_fields.h>
#include <linux/vi_vreg_blocks.h>
...
struct _isp_cfg {
	...
	struct isp_switch_gpio	muxSwitchGpio;
...
struct isp_ctx {
	...
	struct _isp_cfg		isp_pipe_cfg[ISP_PRERAW_VIRT_MAX];
	enum ISP_BAYER_TYPE	rgb_color_mode[ISP_PRERAW_VIRT_MAX];
	uint8_t			rgbmap_prebuf_idx;
	uint8_t			mmap_grid_size[ISP_PRERAW_VIRT_MAX];
	uint8_t			isp_pipe_enable[ISP_PRERAW_VIRT_MAX];
	uint8_t			rawb_chnstr_num;
	uint8_t			total_chn_num;
	atomic_t		is_post_done;

	uint8_t			cam_id;
	uint8_t			gamma_tbl_idx;
	uint32_t		is_multi_sensor     : 1;
...
extern void isp_fill_rgbmap(struct isp_ctx *ctx, enum cvi_isp_raw raw_num, const u8 chn_num);
