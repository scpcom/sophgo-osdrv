...
int vo_create_thread(struct cvi_vo_dev *vdev, enum E_VO_TH th_id)
{
	...
	if (vdev->vo_th[th_id].w_thread == NULL) {
		switch (th_id) {
		case E_VO_TH_DISP:
			strncpy(vdev->vo_th[th_id].th_name, "cvitask_disp", sizeof(vdev->vo_th[th_id].th_name));
...
int vo_stop_streaming(struct cvi_vo_dev *vdev)
{
	...
	if (vdev->shared_mem == NULL) {
		CVI_TRACE_VO(CVI_DBG_ERR, "vdev->shared_mem is NULL\n");
		return -EINVAL;
	}

	voctx = (struct cvi_vo_ctx *)(vdev->shared_mem);
	...
	if (voctx->is_chn_enable[VoLayer][VoChn] == CVI_TRUE) {
		spin_lock_irqsave(&vdev->rdy_lock, flags);
